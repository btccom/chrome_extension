...
var worth,type,oDiv,targetElement,element;
var islink=false;
        ...
        success: function (data) {
            ...
            if (data.error_no == "0") {
                if (result.type == "block") {//高度
                    ...
                    if(result.data.relayed_by=="Unknown"){$("#btc_Relayed").html(result.data.relayed_by)}
                    else{$("#btc_Relayed").html("<a target='_blank' href='https://chain.btc.com/stats/pool/"+result.data.relayed_by+"'>"+result.data.relayed_by+"</a>");}
                }
                if (result.type == "address") {//地址
                    ...
                    if(result.data.unconfirmed_tx_count=="0"){$("#btc_txcount").text(result.data.n_tx)}
                    else{ $("#btc_txcount").text(result.data.n_tx+"("+result.data.unconfirmed_tx_count+" "+dicbook("unconfirmed")+")")};
                ...
                if (result.type == "tx") {//交易
                    ...
                    if(result.data.is_coinbase){ //coinbase 交易
                        ...
                        $("#datainfo").append(tr);
                        if(lng=="zn"){$("#datainfo").addClass("coinbase");}
                        else{$("#datainfo").addClass("coinbase_en");}
                    }
                    if(result.data.confirm<1) { //未确认交易
                        $("#btc_timestamp").text("-");
                        if(lng=="zn"){$("#datainfo").addClass("unconfirm");}
                        else{$("#datainfo").addClass("unconfirm_en");}
                        $("#btc_confirm").text("0");
                        $("#btc_height").text("-");
                    }
                }
                ...
                $(".topline").remove();
                $(".oDivline").remove();$(".btc_logo").remove();
...
var stopEvent = function(event){
    ...
    if(window.event.target.id=="close"){return;}
    else if(window.event.target.id=="oDiv_top"){return;}
